...
async function fileTransfer2G() {
  ...
  fs.readdir(
    `/home/u2000/home/u2000/NBI_PM/pm/pmexport_${cus_modules.dateFunction()}`,
    async (err, data) => {
      try {
        // <*2G*KPI*TEMP*.csv>
        if (err) {
          const errorMessage =
            "<b>Please be informed that The folder from which 3G files are to be copied to the PURA FTP Server for today has not yet been created.</b><br/>" +
            "<b> Thank you.</b>";
          if (cus_modules.detEmailSent()) {
            cus_modules.errorLogging(
              1,
              `Error. /pmexport_${cus_modules.dateFunction()} Folder not found @ ${date}\n`
            );
        ...
        let regex =
          /[a-zA-Z0-9_]*PURA 3G[ a-zA-Z0-9_()]*TEMP[a-zA-Z0-9()]*.csv/; // Regular Epression to get all files with 3G
        ...
        const errorMessage =
          "<b>Please be informed that The folder from which 2G files are to be copied to the PURA FTP Server for today has not yet been created.</b><br/>" +
          "<b> Thank you.</b>";
        if (cus_modules.detEmailSent()) {
          cus_modules.errorLogging(
            1,
  ...
  try {
    ...
    const errorMessage =
      "<b>Please be informed that The Pura FTP Server cannot be reached.</b><br/>" +
      "<b> Thank you.</b>";
    cus_modules.errorLogging(
      1,
      `Error. CONNECTION TO SERVER GOT INTERRUPTED WHILE COPYING FILES @ ${date}\n`
    );
    await cus_modules.sendMail(errorMessage); // send email notification.
  ...
  while (true) {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        cus_modules.errorLogging(
          2,
          `${fileToMove} SUCCESSFULLY COPIED. @ ${date}\n`
        );
        ...
        cus_modules.fileBackup(
          `/home/u2000/home/u2000/NBI_PM/pm/pmexport_${cus_modules.dateFunction()}/${file2Delete}`,
          file2Delete,
          `/home/u2000/ftpFileBackUp/pura_${cus_modules.dateFunction()}`
        ); // BACKING UP FILES
      }
      if (files.length == 0) {
  ...
  if (files.length > 0) {
    ...
    // EMAIL ERROR MESSAGE
    const errorMsg =
      "<b>Please be informed that There Are some Files Tht couldn't be transfered to the via ftp to the Pura Server.</b>" +
